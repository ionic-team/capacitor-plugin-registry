...
import { Breakpoint, Column, Grid, HighlightedCode, HubspotForm, Modal } from '@ionic-internal/components-react';
    ...
    <section className={styles.top} id="top">
      ...
      <div className="ds-container">
        ...
        <div className="image-wrapper">
          <Image src={ImageTop0} width={ImageTop0.width / 2} height={ImageTop0.height} alt="multi layered phone" />
        </div>
      </div>
    </section>
  );
...
const Announcement = () => {
  ...
  return (
    ...
    >
      ...
      <Breakpoint visible={[0, 1, 1, 1, 1]} display="inline-block" className="text">
        <span className="text__content">{desktop_text} →</span>
      </Breakpoint>
      <Breakpoint visible={[1, 0, 0, 0, 0]} display="inline-block" className="text">
...
const Ebook = () => {
  const { ebookModalOpen, setEbookModalOpen, prismicData } = useContext(IndexPageContext) || {};
  if (!prismicData) return null;
...
const Framework = () => {
  ...
  const logoTile = (logo: any) => <PrismicImage2x field={logo} width="272" height="200" />;
...
const Tweets = () => {
  const { prismicData } = useContext(IndexPageContext) || {};
  if (!prismicData) return null;
  const { tweets, tweets__list, tweets__bottom, tweets__bottom__list } = prismicData;
...
const Cta = () => {
  const { prismicData } = useContext(IndexPageContext) || {};
  if (!prismicData) return null;
  const { get_started2: get_started, get_started__ctas, companies__list2, cta } = prismicData;
    ...
    <section className={styles.multisection} id="multisection">
      <div className="ds-container">
        <div className={styles.cta} id="cta">
          ...
          <div className="wrapper">
            <div className="card">
              <LegacyPrismicResponsiveImage image={image} className="background" />
              <div className="heading-group">
                <PrismicRichText
                  field={text}
                  components={{
